...
import exampleImage from "../../../images/profile2.png";
...
function AnswerPage({ allQuestions }) {
  ...
  // ////////GETING THE ANSWER DATA BASED ON THE FILTERDQUESTIONID////
  const [allAnswers, setAllAnswers] = useState([]);

  const fetchAnswers = async () => {
    try {
      const answerRes = await axios.get(
        // /sending the question id as a params(url)
        `http://localhost:3000/api/answer/${filteredquestionId}`,
        {
          headers: { "x-auth-token": userData.token },
        }
      );
      // ///putting/updating the data on the declared variable
  ...
  // //navihates it to login page if they did not login in
  useEffect(() => {
